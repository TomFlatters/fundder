...
exports.onRefreshChallenge = functions.https.onCall(async (data, context)=>{
  ...
  const query = await postsCollection.where("challengeId", "==", challenge).orderBy("timestamp", 'desc').startAfter(timeStamp).limit(limit).get();
  const queryDocSnap = query.docs
  const queryData = queryDocSnap.map((qDocSnap)=> qDocSnap.data())
